...
class Encoder: NSObject {
    ...
    class func prepareEncodedResponse(encodedArray: inout [UInt8]) -> String {
        ...
        response.append(Data(encodedArray).binary_decimal.hex)
        return response
    }
...
extension NSNumber {
    @objc override func encode() -> String {
        let major: MajorType = (intValue < 0) ? .major1 : .major0
        let measure = (intValue < 0) ? (intValue * -1) - 1 : intValue
...
extension NSArray {
    @objc override func encode() -> String {
        let encodedArray = Encoder.prepareByteArray(major: .major4, measure: count)
...
extension NSDictionary {
    @objc override func encode() -> String {
        let encodedArray = Encoder.prepareByteArray(major: .major5, measure: allKeys.count)
        return (Data(encodedArray).binary_decimal.hex).appending(getItemsEncoding())
    }
